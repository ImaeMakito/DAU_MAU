...
with whole as (
  ...
  where
    _table_suffix between  '20180210' and '20180315' and
    type != 'register' and 
    type != 'discovery_contents' and
    type !=  'recommendation_list'

    -- registerログは、１ユーザーに対して複数発行される可能性があるため、ログインUUのカウントから除外する
    --descovery_contents と　recomendation_list も上記と同じく除外する（大量のログが発生している原因要素）
  union all
  select distinct
    user_id,
    platform as os,
  ...
  where
    _table_suffix between  '20180210' and '20180315'
